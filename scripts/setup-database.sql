...
-- Create enrollments table (tracks student progress)
...
-- Create quiz answers (for multiple choice)
...
-- Create violations (strikes for academic integrity)
...
-- Create exam answers (for multiple choice)
...
-- Create exam responses (student answers)
...
-- Create grades table (overall course grades)
...
-- RLS Policies: Instructors can see courses they created
CREATE POLICY "Instructors can view their courses" ON courses
  FOR SELECT USING (instructor_id = auth.uid());

-- RLS Policies: Students can see enrollments
...
-- RLS Policies: Students can see lab submissions
CREATE POLICY "Users can see their lab submissions" ON lab_submissions
  FOR SELECT USING (
    student_id = auth.uid() OR 
    lab_id IN (
